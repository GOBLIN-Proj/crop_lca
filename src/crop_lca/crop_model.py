...
class Emissions_Factors(object):
    def __init__(self, data):
        ...
        for _, row in self.data_frame.iterrows():
            ...
            ef_country = row.get("ef_country".lower())
            ...
            self.emissions_factors[ef_country] = {
    ...
    def get_ef_emissions_factor_1_ipcc_2019(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get(
            "ef_emissions_factor_1_ipcc_2019"
        )

    def get_ef_urea(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get("ef_urea")

    def get_ef_urea_and_nbpt(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get("ef_urea_and_nbpt")

    def get_ef_fracGASF_urea_fertilisers_to_nh3_and_nox(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get(
            "ef_fracGASF_urea_fertilisers_to_nh3_and_nox"
        )

    def get_ef_fracGASF_urea_and_nbpt_to_nh3_and_nox(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get(
            "ef_fracGASF_urea_and_nbpt_to_nh3_and_nox"
        )

    def get_ef_frac_leach_runoff(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get("ef_frac_leach_runoff")

    def get_ef_ammonium_nitrate(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get("ef_ammonium_nitrate")

    def get_ef_fracGASF_ammonium_fertilisers_to_nh3_and_nox(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get(
            "ef_fracGASF_ammonium_fertilisers_to_nh3_and_nox"
        )
    ...
    def get_ef_indirect_n2o_from_leaching_and_runoff(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get(
            "ef_indirect_n2o_from_leaching_and_runoff"
        )

    def get_ef_grassland_dm_t(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get("ef_grassland_dm_t")

    def get_ef_dm_carbon_stock_crops(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get(
            "ef_dm_carbon_stock_crops"
        )

    def get_ef_Frac_P_Leach(self, emissions_factors):
        return self.emissions_factors.get(emissions_factors).get("ef_Frac_P_Leach")
...
class Fertiliser(object):
    ...
    def get_fert_kg_n_per_ha(self, fertiliser):
        return self.fertiliser.get(fertiliser).get("fert_kg_n_per_ha")

    def get_fert_kg_p_per_ha(self, fertiliser):
        return self.fertiliser.get(fertiliser).get("fert_kg_p_per_ha")

    def get_fert_kg_k_per_ha(self, fertiliser):
        return self.fertiliser.get(fertiliser).get("fert_kg_k_per_ha")
...
def print_crop_data(data):
    for _, key in enumerate(data):
        for crop in data[key].keys():
            for crop_type in data[key][crop_type].__dict__.keys():
                for attribute in (
                    data[key][crop].__getattribute__(crop_type).__dict__.keys()
                ):
                    print(
                        f"{crop_type}: {attribute} = {data[key][crop].__getattribute__(crop_type).__getattribute__(attribute)}"
                    )

def print_crop_data_test(data):
    for _, key in enumerate(data):
